...
const categoryMap = {
  logica: 'Logica', matematica: 'Matematica', attenzione: 'Attenzione', interruttori: 'Interruttori',
  fiammiferi: 'Fiammiferi', porte: 'Osservazione', bicchieri: 'Bicchieri', detective: 'Detective',
  sequenze: 'Sequenze', parole: 'Parole'
};
...
const proceduralGenerators = {
  attenzione: () => {
    const templates = [
      () => {
        const n = rnd(7, 45), left = rnd(2, n - 2), obj = pick([...data.objects, ...data.animals]);
      ...
      () => {
        ...
        return { category: 'Attenzione', difficulty: 'medio', title: 'Sole o buio?', text: `Se piove a mezzanotte, è possibile che dopo ${hours} ore ci sia il sole?`, solution: 'No, sarà di nuovo mezzanotte se le ore sono multipli di 24.', hook: 'Conta le ore, non rispondere a sensazione.' };
      }
    ];
  ...
  matematica: () => {
    const templates = [
      () => {
        const machines = pick([3, 4, 5, 6, 8, 10]), target = machines * pick([8, 10, 15, 20, 25]), minutes = pick([3, 5, 6, 10, 12]);
      ...
      () => {
        const child = rnd(3, 18), diff = pick([18, 20, 22, 24, 26, 30, 32, 36]), total = child + child + diff;
      ...
      () => {
        const price = pick([20, 30, 40, 50, 60, 80, 100, 120]), discount = pick([10, 20, 25, 30, 50]), finalPrice = price * (100 - discount) / 100;
  ...
  logica: () => {
    const obj = pick(data.objects), n = rnd(10, 50), take = rnd(3, n - 3);
  ...
  sequenze: () => {
    const start = rnd(1, 6), step = rnd(2, 9), seq = Array.from({ length: 5 }, (_, i) => start + i * step);
...
function getBankCandidates() { return (window.QUIZ_BANK || []).filter(matchesFilters); }
...
function uniqueQuiz() {
  const combined = [...shuffle(getBankCandidates())];
  ...
  state.recent.push(quizKey(quiz));
...
function recommendedDuration(quiz) {
  const base = quiz.text.length < 120 ? 10 : quiz.text.length < 210 ? 14 : 18;
  return quiz.difficulty === 'difficile' ? base + 6 : base;
}
function buildVideoPlan(quiz, score) {
  const seconds = recommendedDuration(quiz);
  return `Durata consigliata: ${seconds}-${seconds + 4}s\nStruttura: 1) hook iniziale 2s, 2) domanda 6-10s, 3) pausa per commenti, 4) soluzione nel commento fissato.\nViralità stimata: ${score}/100`;
}
...
function buildPinnedComment(quiz) { return `✅ SOLUZIONE:\n${quiz.solution}\n\nCi eri arrivato? 👇`; }
function buildVoiceover(quiz) {
  return `Aspetta, non rispondere di getto.\n${quiz.text}\nHai pochi secondi per pensarci.\nScrivi la risposta nei commenti prima di guardare la soluzione.`;
}
...
function updateStats(score = 0) {
  ...
  $('statVariants').textContent = (total * 250 + 50000).toLocaleString('it-IT');
...
function generateQuiz() {
  ...
  $('videoPlan').textContent = buildVideoPlan(quiz, score);
  $('description').value = buildDescription(quiz);
  $('pinnedComment').value = buildPinnedComment(quiz);
  $('voiceover').value = buildVoiceover(quiz);
...
async function copyText(text, buttonId, label) {
  ...
  } catch { alert('Copia non riuscita: seleziona il testo manualmente.'); }
}
...
$('copyVoiceBtn').addEventListener('click', () => copyFrom('voiceover', 'copyVoiceBtn', 'Copia voice-over'));
